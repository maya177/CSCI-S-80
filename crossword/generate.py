...
class CrosswordCreator():
    ...
    def backtrack_ac3(self, assignment):
        ...
        # if assignment is already complete (all variables have words) return it
        if len(assignment) == len(self.domains):
            return assignment

        # selecting one of unassigned variables to choose its domain
        variable = self.select_unassigned_variable(assignment)

        # iterating through words in that variable that was sorted with heuristics
        for value in self.domains[variable]:
            # assign the variable selected to assignment to see if it fits
            assignment[variable] = value
            # if the assignment remains consistent - all words unique and overlaps same characters
            if self.consistent(assignment):
                # recursive call to see if further values that arise from this choice are consistent
                result = self.backtrack(assignment)
                if result: 
                    return result
                
                # if no assignment received, remove variable
